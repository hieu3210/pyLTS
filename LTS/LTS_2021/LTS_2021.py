# High-Order Linguistic Time Series forecasting model with growth of word-set
from HAs import HedgeAlgebras


class HighOrderLTS:
    # Constructor
    def __init__(self, order, repeat, data, lb, ub, theta, alpha, max_length=3, grow_wordset=False):
        ...
        self.grow_wordset = grow_wordset  # If True, grow word-set based on data
    ...
    def get_words(self):
        if self.grow_wordset:
            # Grow word-set based on data size or something, but for simplicity, use max_length
            return self.ha.get_words(self.max_length)
        else:
            return self.ha.get_words(self.max_length)
    ...
    # Get linguistic logical relationship groups (high-order)
    def get_rules(self):
